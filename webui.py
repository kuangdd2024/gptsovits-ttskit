...
if 'Path' in os.environ:
    os.environ['Path'] += f';{now_dir}'
else:
    os.environ['Path'] = now_dir
    ...
    for i in range(ngpu):
        ...
        if any(value in gpu_name.upper() for value in
            ...
            try:
                ...
                gpu_mem = meminfo.used  # total
            mem.append(int(gpu_mem / 1024 / 1024 / 1024 + 0.4))
# # 判断是否支持mps加速
...
def change_label(if_label, path_list):
    ...
    if (if_label == True and p_label == None):
        ...
        yield i18n(f"打标工具WebUI已开启，地址：\nhttp://vc3ann.kddbot.com")#http://{webui_host}:{webui_port_subfix - 3840}")
        # 9871 6031
        print(cmd)
...
def change_uvr5(if_uvr5, corpus_name):
    global p_uvr5
    if (if_uvr5 == True and p_uvr5 == None):
        cmd = '"%s" tools/uvr5/webui.py "%s" %s %s %s %s' % (
            python_exec, infer_device, is_half, webui_port_uvr5, is_share, corpus_name)
        yield i18n(f"UVR5已开启，地址：\nhttp://vc3uvr.kddbot.com")#http://{webui_host}:{webui_port_uvr5 - 3840}")
        print(cmd)
...
def change_tts_inference(if_tts, bert_path, cnhubert_base_path, gpu_number, gpt_path, sovits_path, corpus_name):
    ...
    if (if_tts == True and p_tts_inference == None):
        ...
        os.environ["corpus_name"] = str(corpus_name)
        os.environ["PYTORCH_NVML_BASED_CUDA_CHECK"] = "1"
        cmd = '"%s" GPT_SoVITS/inference_webui.py' % (python_exec)
        yield i18n(f"TTS推理进程已开启，地址：\nhttp://vc3tts.kddbot.com")#http://{webui_host}:{webui_port_infer_tts - 3840}")
        print(cmd)
...
def load_corpus(name, files, opts, audio, video):
    """
    接收多个文件并保存到本地的'saved_files'目录。
    """
    ...
    name = name or os.path.basename(files[0])
    name = re.sub(r'[^a-zA-Z0-9\u4E00-\u9FA5]', '', name)

    corpus_path = f'output/{name}/corpus'
    os.makedirs(corpus_path, exist_ok=True)

    wav_opt_path = f'output/{name}/wav_opt'
    os.makedirs(wav_opt_path, exist_ok=True)

    for idx, inpath in enumerate(tqdm.tqdm(files, 'load_corpus', ncols=80), 1):
        file_name = os.path.join(corpus_path, os.path.basename(inpath))
        shutil.copyfile(inpath, file_name)

        wav_path = os.path.join(wav_opt_path, f'norm-{name}-{idx:03d}.wav')
        if '00-语音标准化' in opts:
            seg = pydub.AudioSegment.from_file(inpath)
            seg = seg.set_channels(1).set_frame_rate(48000).set_sample_width(2)
            seg.export(wav_path, format='wav')
        else:
            shutil.copyfile(inpath, wav_path)

    corpus_info = f"已保存文件到目录：{corpus_path}，标准化后保存wav文件目录：{wav_opt_path}"
    uvr5_opt_path = f'output/{name}/uvr5_opt'
    slice_inp_path = uvr5_opt_path if '0a-人声提取' in opts else wav_opt_path
    slice_opt_root = f'output/{name}/slicer_opt'
    denoise_input_dir = slice_opt_root if '0b-语音切分' in opts else slice_inp_path
    denoise_output_dir = f'output/{name}/denoise_opt'
    asr_inp_dir = denoise_output_dir if '0bb-语音降噪' in opts else denoise_input_dir
    asr_opt_dir = f'output/{name}/asr_opt'
    path_list = f'output/{name}/asr_opt/{os.path.basename(asr_inp_dir)}.list'
    inp_text = path_list
    inp_wav_dir = asr_inp_dir
    exp_name = name
    return [corpus_info,
            slice_inp_path, slice_opt_root,
            denoise_input_dir, denoise_output_dir,
            asr_inp_dir, asr_opt_dir,
            path_list,
            inp_text, inp_wav_dir, exp_name]

#账户和密码相同就可以通过
def auth_judge(username, password):
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    ...
    gr.Markdown(
        value=
        i18n("语料可选一或多种形式导入，后台会汇总处理")
    )
    with gr.Row():
        with gr.Column():
            corpus_name = gr.Textbox(label=i18n("名称代号（推荐字母或字母数字组合）"), value="")
            corpus_opt_list = gr.CheckboxGroup(label="请选择后续需要的操作",
                                               choices=['00-语音标准化', '0a-人声提取', '0b-语音切分', '0bb-语音降噪'],
                                               value=['00-语音标准化', '0b-语音切分'])

        corpus_list = gr.File(file_count="multiple",
                              label="上传音视频文件，支持多个文件，格式支持：wav，mp3，mp4等")
        corpus_audio = gr.Audio(
            sources=["upload", "microphone"],
            type="filepath",
            label="麦克风录音或上传语音文件",
            visible=True
        )
        corpus_video = gr.Video(
            sources=["upload", "webcam"],
            label="摄像头拍摄或上传视频文件",
            visible=True
        )
    with gr.Row():
        corpus_load_button = gr.Button(i18n("导入数据"), variant="primary", visible=True)
        corpus_info = gr.Textbox(label=i18n("输出信息"))
    with gr.Tabs():
        with gr.TabItem(i18n("0-前置数据集获取工具")):  # 提前随机切片防止uvr5爆内存->uvr5->slicer->asr->打标
            ...
            with gr.Row():
                with gr.Row():
                    ...
                    hop_size = gr.Textbox(
                        label=i18n("hop_size:怎么算音量曲线，越小精度越大计算量越高（不是精度越大效果越好）"), value="10")
                ...
                with gr.Row():
                    ...
                    _max = gr.Slider(minimum=0, maximum=1, step=0.05, label=i18n("max:归一化后最大值多少"), value=0.9,
                                     interactive=True)
                    alpha = gr.Slider(minimum=0, maximum=1, step=0.05,
                                      label=i18n("alpha_mix:混多少比例归一化后音频进来"), value=0.25, interactive=True)
                    n_process = gr.Slider(minimum=1, maximum=n_cpu, step=1, label=i18n("切割使用的进程数"), value=4,
                                          interactive=True)
            ...
            if_uvr5.change(change_uvr5, [if_uvr5, corpus_name], [uvr5_info])
            open_asr_button.click(open_asr, [asr_inp_dir, asr_opt_dir, asr_model, asr_size, asr_lang],
                                  [asr_info, open_asr_button, close_asr_button])
            close_asr_button.click(close_asr, [], [asr_info, open_asr_button, close_asr_button])
            open_slicer_button.click(open_slice,
                                     [slice_inp_path, slice_opt_root, threshold, min_length, min_interval, hop_size,
                                      max_sil_kept, _max, alpha, n_process],
                                     [slicer_info, open_slicer_button, close_slicer_button])
            close_slicer_button.click(close_slice, [], [slicer_info, open_slicer_button, close_slicer_button])
            open_denoise_button.click(open_denoise, [denoise_input_dir, denoise_output_dir],
        ...
        with gr.TabItem(i18n("1-GPT-SoVITS-TTS")):
            ...
            with gr.TabItem(i18n("1A-训练集格式化工具")):
                ...
                with gr.Row():
                    inp_text = gr.Textbox(label=i18n("*文本标注文件"), value=r"D:\RVC1006\GPT-SoVITS\raw\xxx.list",
                                          interactive=True)
                ...
                with gr.Row():
                    gpu_numbers1a = gr.Textbox(label=i18n("GPU卡号以-分割，每个卡号一个进程"),
                                               value="%s-%s" % (gpus, gpus), interactive=True)
                ...
                with gr.Row():
                    gpu_numbers1Ba = gr.Textbox(label=i18n("GPU卡号以-分割，每个卡号一个进程"),
                                                value="%s-%s" % (gpus, gpus), interactive=True)
                ...
                with gr.Row():
                    gpu_numbers1c = gr.Textbox(label=i18n("GPU卡号以-分割，每个卡号一个进程"),
                                               value="%s-%s" % (gpus, gpus), interactive=True)
            ...
            button1a_open.click(open1a, [inp_text, inp_wav_dir, exp_name, gpu_numbers1a, bert_pretrained_dir],
                                [info1a, button1a_open, button1a_close])
            button1a_close.click(close1a, [], [info1a, button1a_open, button1a_close])
            button1b_open.click(open1b, [inp_text, inp_wav_dir, exp_name, gpu_numbers1Ba, cnhubert_base_dir],
                                [info1b, button1b_open, button1b_close])
            button1b_close.click(close1b, [], [info1b, button1b_open, button1b_close])
            button1c_open.click(open1c, [inp_text, exp_name, gpu_numbers1c, pretrained_s2G],
                                [info1c, button1c_open, button1c_close])
            button1c_close.click(close1c, [], [info1c, button1c_open, button1c_close])
            button1abc_open.click(open1abc,
                                  [inp_text, inp_wav_dir, exp_name, gpu_numbers1a, gpu_numbers1Ba, gpu_numbers1c,
            ...
            with gr.TabItem(i18n("1B-微调训练")):
                ...
                with gr.Row():
                    ...
                    total_epoch = gr.Slider(minimum=1, maximum=25, step=1,
                                            label=i18n("总训练轮数total_epoch，不建议太高"), value=8, interactive=True)
                ...
                with gr.Row():
                    batch_size1Bb = gr.Slider(minimum=1, maximum=40, step=1, label=i18n("每张显卡的batch_size"),
                                              value=default_batch_size, interactive=True)
                    total_epoch1Bb = gr.Slider(minimum=2, maximum=50, step=1, label=i18n("总训练轮数total_epoch"),
                                               value=15, interactive=True)
                    ...
                    if_save_every_weights1Bb = gr.Checkbox(
                        label=i18n("是否在每次保存时间点将最终小模型保存至weights文件夹"), value=True, interactive=True,
                        show_label=True)
                    save_every_epoch1Bb = gr.Slider(minimum=1, maximum=50, step=1,
                                                    label=i18n("保存频率save_every_epoch"), value=5, interactive=True)
            ...
            button1Ba_open.click(open1Ba, [batch_size, total_epoch, exp_name, text_low_lr_rate, if_save_latest,
                                           if_save_every_weights, save_every_epoch, gpu_numbers1Ba, pretrained_s2G,
                                           pretrained_s2D], [info1Ba, button1Ba_open, button1Ba_close])
            button1Ba_close.click(close1Ba, [], [info1Ba, button1Ba_open, button1Ba_close])
            button1Bb_open.click(open1Bb, [batch_size1Bb, total_epoch1Bb, exp_name, if_dpo, if_save_latest1Bb,
                                           if_save_every_weights1Bb, save_every_epoch1Bb, gpu_numbers1Bb,
                                           pretrained_s1], [info1Bb, button1Bb_open, button1Bb_close])
            ...
            with gr.TabItem(i18n("1C-推理")):
                ...
                with gr.Row():
                    GPT_dropdown = gr.Dropdown(label=i18n("*GPT模型列表"),
                                               ...
                                               value=pretrained_gpt_name, interactive=True)
                    SoVITS_dropdown = gr.Dropdown(label=i18n("*SoVITS模型列表"),
                                                  choices=sorted(SoVITS_names, key=custom_sort_key),
                                                  value=pretrained_sovits_name, interactive=True)
                ...
                with gr.Row():
                    ...
                    if_tts.change(change_tts_inference,
                                  [if_tts, bert_pretrained_dir, cnhubert_base_dir, gpu_number_1C, GPT_dropdown,
                                   SoVITS_dropdown, corpus_name], [tts_info])
        with gr.TabItem(i18n("2-GPT-SoVITS-变声")):
            gr.Markdown(value=i18n("施工中，请静候佳音"))

        corpus_load_button.click(load_corpus, [corpus_name, corpus_list, corpus_opt_list, corpus_audio, corpus_video],
                                 [corpus_info,
                                  slice_inp_path, slice_opt_root,
                                  denoise_input_dir, denoise_output_dir,
                                  asr_inp_dir, asr_opt_dir,
